    ...
    if sv.isFusion:
        ...
        if sv.svtype == "TRANSLOCATION":
            ...
            sv.annotationPartner1.variantSite2, sv.annotationPartner2.variantSite2 = \
                [sv.annotationPartner2.pos]*2
        else:
            sv.bkpsites = get_bkpsite_note(
                sv, sv.annotationPartner1, sv.annotationPartner2)
            note1 = sv.annotationPartner1.gene + " " + \
...
def get_kinase_status(bkp, kinase_annotation):
    ...
    try:
        ...
        e = ("Breakpoint site positions cannot be determined "
...
def get_misc_notes(sv):
    ...
    if sv.isIntragenic:
        ...
        kinase1, kinase2 = map(lambda x: get_kinase_note(x),
                               (sv.annotationPartner1, sv.annotationPartner2))
    if all([kinase1, kinase2, sv.annotationPartner1.isEntireKinase ==
        ...
        kinase1 = kinase1.replace(
            "domain", "domains").replace("a part", "parts")
...
def clinical_warning_note(sv):
    bkps_involved = filter(
        ...
        [sv.annotationPartner1, sv.annotationPartner2])
    bkps_genes = list(set(map(
        lambda x: x.gene, bkps_involved)))
...
def functional_significance(sv):
    ...
    if sv.isKnownFusion:
        pass
    elif all([sv.annotationPartner1.isKinase,
              sv.annotationPartner1.isHotspot]) or \
            all([sv.annotationPartner2.isKinase,
                 sv.annotationPartner2.isHotspot]):
        func_sig_note = clinical_warning_note(sv)
    elif sv.svtype == "DELETION" and \
        any([all([sv.bkp1.isPanel, sv.bkp1.isCoding,
                  sv.bkp1.isTumourSuppressor]),
             all([sv.bkp2.isPanel, sv.bkp2.isCoding,
                  sv.bkp2.isTumourSuppressor])]):
        pass
    else:
        func_sig_note = \
            " Functional significance is undetermined."
...
def special_cases(sv):
    ...
    if sv.annotation.startswith("EGFR (NM_005228) rearrangement:"):
        ...
        cdkn2a_tx = filter(lambda x: isinstance(x, str),
...
def override_fusion(sv):
    if not sv.isFusion:
        ...
        if any([
            ...
            all([sv.fusionPartner2.isKinase,
...
def get_notes(sv, refFlat_summary, kinase_annotation):
    ...
    map(lambda bkp: get_kinase_status(bkp, kinase_annotation),
        (sv.bkp1, sv.bkp2))
